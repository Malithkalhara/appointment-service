...
class CustomerController {
  ...
  // Check if a customer exists by mobile number, if not, create a new customer
  async checkCustomerExists(req, res) {
    try {
      const { mobileNo } = req.params;
      const customer = await CustomerModel.getCustomerByMobileNo(mobileNo);

      if (customer) {
        res.status(200).json({ message: "Customer exists", customer });
      } else {
        // Customer does not exist, you can create a new one
        ...
        res
          .status(201)
          .json({ message: "New customer created", customer: newCustomer });
      }
    } catch (error) {
      res
        .status(500)
