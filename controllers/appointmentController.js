...
class AppointmentController {
    ...
    async createAppointment(req, res) {
      try {
        const { title, details, customerId, itemIds, date, timeDuration  } = req.body;
        const appointment = await AppointmentModel.createAppointment(title, details, customerId, itemIds, date, timeDuration);
    ...
    async updateAppointment(req, res) {
      try {
        const { appointmentId } = req.params;
        const { title, details, itemIds, date, timeDuration } = req.body;
        const updatedAppointment = await AppointmentModel.updateAppointment(appointmentId, title, details, itemIds, date, timeDuration);
